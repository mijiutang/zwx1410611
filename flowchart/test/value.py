        ...
        with sync_playwright() as p:
            ...
            if save_target_contents:
                ...
                if crawl_type == "left":
                    ...
                    # 生成输出文件名 - 保存到test目录，使用sessionIndex作为文件名
                    test_dir = r"C:\Users\1\Desktop\workfast\zwx1410611\flowchart\test"
                    # 确保目录存在
                    os.makedirs(test_dir, exist_ok=True)
                    
                    # 命名规则改为：使用sessionIndex作为文件名
                    right_output_file = os.path.join(test_dir, f"{session_index}.json")
